...
Adapted 07 Sep 2022 for Multi-axis tomography by George R Lewis
Find changes by searching 'Changed by G.R. Lewis'
'''
...
class tomo_data(ndarray):
    def __new__(cls, arr, vectors, stack_dim=None, tilt_axis=0,
...
#                 raise NotImplementedError
        ...
        # Changed by G.R. Lewis
        # We will define our input angles using astra's 'vector' framework rather than simple angles
...
#             raise NotImplementedError
        ...
        if backend == 'astra':
            ...
            vol_geom = astra.creators.create_vol_geom(vol_shape[1],vol_shape[0],vol_shape[2])
...
# #                             'parallel' + dim_str, *self.det, -self.angles)
                ...
                # Changed by G.R. Lewis
                # Providing the projection geometry with vectors rather than angles to make it multi-axis
                proj_geom = astra.create_proj_geom('parallel3d_vec', vol_shape[0],vol_shape[2],self.vectors)
...
class SART(tomo_alg):
    def __init__(self, op=None, iterations=None, min_val=None, max_val=None):
        '''
...
class PDHG(tomo_iter_alg):
    ...
    def step(self, i, niter): # the algorithm basically just repeats step x number of times
        for _ in range(niter):
            # Primal step:
            tmp = self.x
            # uses previously defined proximity function to determine update (based on L1 or L2 etc)
            self.x = self.f.prox(self.x - self.t * self.Ay) # x updates according to y
            self.xm1, self.Axm1 = tmp, self.Ax
            self.Ax = self.A * self.x

            # Dual step:
            tmp = self.y
            self.y = self.g.prox(self.y + self.s * (2 * self.Ax - self.Axm1)) # y updates according to x
...
class TV(PDHG):
    ...
    def getRecon(self):# x ends up being the reconstructed object
        return self.x[0].reshape(self.d.vol_shape) * self.dataScale
    
class TV_unnorm(PDHG):
    def __init__(self, shape, order=1,
                 op=None, weight=0.1, pos=True):
        '''
        Changed by G.R. Lewis
        
        As TV(), but data is NOT normalised between 0 and 1
        ...
        PDHG.__init__(self, None,
                      stackProxFunc(ZERO()), 
                      None, op=op) # changed to just zero
        self.pos = pos
        self.d = diff(shape, order=order)
        self.order = order
        self.weight = weight
    ...
    def setParams(self, data, op=None, x=None, y=None, **kwargs):
        ...
        PDHG.setParams(self, A=A,
                       f=stackProxFunc(ZERO()), # changed this to just zero
                       g=gstar.dual, x=x,
                       y=y, normA=normA, **kwargs)
...
class WaveletL1(L1_2):
    def __init__(self, scale=1, translation=None, wavelet='haar', mode='constant', vol_shape=None, spect_shape=1):
        ...
        if wavelet in (None, 'None', 'none'):  # No wavelet, fwrd/bwrd are identity
            ...
            #assert self.wavelet[0].orthogonal # Non-orthogonal wavelets perform much worse!
...
class Wavelet(FISTA):
    ...
    def setParams(self, data, op=None, x=None, **kwargs):
        ...
        ### Changed by G.R. Lewis
        # I changed the second argument from 1e-6 to 1e-20
        # I believe it sets a minimum bound on expected data scale, which for A is regularly ~ 1e14
        self.dataScale = max(max(abs(w).max() for w in g.fwrd(R.T.dot(data.ravel()), listify=True)), 1e-20)
...
#     rec2d = sirt.run(data=sino2d, backend='astra', min_val=0)reconstructions
